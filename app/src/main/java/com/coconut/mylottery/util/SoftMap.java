...
public class SoftMap<K, V> extends HashMap<K, V> {
	...
	private HashMap<K, MySoftReference<K, V>> temp;// 存放袋子的集合
	...
	public SoftMap() {
		...
		temp = new HashMap<K, MySoftReference<K, V>>();
	...
	public V put(K key, V value) {
		...
		MySoftReference<K, V> sr = new MySoftReference<K, V>(key, value, queue);
		temp.put(key, sr);
		return null;
	}
